...
class TestMetaCompatibility:
        ...
        data = pd.DataFrame(
            {
                ...
                "diagnosis": ["ClassA"] * 10
                + ["ClassB"] * 9
                + ["ClassC"],  # ClassC has only 1 sample
            }
        )
    ...
    def test_class_filtering(self, temp_csv):
        ...
        csv_path = temp_csv(data)
        config = MetaConfig(
            data_csv=csv_path, label_col="diagnosis", classes=["ClassA", "ClassB", "ClassC"]
        )
...
class TestHierarchicalCompatibility:
    ...
    def test_valid_hierarchical_data(self, temp_csv):
        ...
        config = HierarchicalConfig(
            data_csv=csv_path, patient_col="svs_id", label_l1="tumor_type", label_l2="subtype"
        )
    ...
    def test_missing_patient_column(self, temp_csv):
        ...
        csv_path = temp_csv(data)
        config = HierarchicalConfig(data_csv=csv_path, patient_col="svs_id", label_l1="diagnosis")
    ...
    def test_too_few_patients_for_cv(self, temp_csv):
        ...
        csv_path = temp_csv(data)
        config = HierarchicalConfig(
            data_csv=csv_path, patient_col="svs_id", label_l1="diagnosis", outer_folds=3
        )
        ...
        data = pd.DataFrame(
            {
                ...
                "subtype": ["SubA1"] * 15
                + ["SubB1"] * 8
                + ["SubB2"] * 7,  # TypeA has 1 L2, TypeB has 2
            }
        )
    ...
    def test_file_not_found(self):
        ...
        config = MetaConfig(data_csv=Path("/nonexistent/file.csv"), label_col="diagnosis")
...
class TestCompatibilityResult:
    ...
    def test_incompatible_result_format(self):
        ...
        result = CompatibilityResult(
            is_compatible=False, mode="meta", errors=["Critical error"], suggestions=["How to fix"]
        )
