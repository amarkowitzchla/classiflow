...
def create_controlled_dataset(tmp_path: Path) -> Tuple[Path, Path]:
    ...
    X = pd.DataFrame(np.vstack(X_data), columns=["feat_0", "feat_1", "feat_2", "feat_3"])
...
def trace_class_order_through_inference(test_path: Path, training_dir: Path, inference_dir: Path):
    ...
    meta_model, meta_features, meta_classes, calibration_metadata = loader.load_meta_artifacts(
        variant="none"
    )
...
def test_full_pipeline_class_alignment():
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        # Critical assertions
        assert (
            training_info["meta_classes"] == training_info["model_classes"]
        ), "meta_classes.csv != model.classes_"

        assert (
            inference_info["loaded_meta_classes"] == inference_info["proba_classes"]
        ), "Loaded meta_classes != probability column classes"

        assert (
            inference_info["argmax_mismatches"] == 0
        ), f"argmax mismatches: {inference_info['argmax_mismatches']}"

        assert (
            abs(brier_info["computed_brier"] - brier_info["manual_brier"]) < 1e-6
        ), "Brier score mismatch"

        assert inference_info["accuracy"] > 0.5, f"Accuracy collapsed: {inference_info['accuracy']}"
