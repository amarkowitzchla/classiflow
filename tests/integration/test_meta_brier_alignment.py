...
def test_brier_with_misaligned_classes():
    ...
    assert not np.isclose(wrong_brier, 0.0), \
...
def test_brier_compute_function_uses_passed_classes():
    ...
    assert not np.isclose(metrics_correct['brier'], metrics_wrong['brier']), \
        "Brier scores should differ with different class orders"

    # Correct order should give lower (better) Brier
    assert metrics_correct['brier'] < metrics_wrong['brier'], \
...
def test_inference_class_order_extraction():
    ...
    assert np.isclose(metrics['brier'], manual_brier), \
        f"Function Brier {metrics['brier']} != manual Brier {manual_brier}"
...
def test_meta_predictor_class_order():
    ...
    assert actual_classes == saved_classes, \
