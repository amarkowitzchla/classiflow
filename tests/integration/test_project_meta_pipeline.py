...
def compare_artifacts(technical_run: Path, final_run: Path) -> Dict[str, Any]:
    ...
    if tech_fold_dir.exists():
        ...
        if classes_path.exists():
            result["technical_classes"] = (
                pd.read_csv(classes_path, header=None).iloc[:, 0].astype(str).tolist()
            )

        features_path = tech_fold_dir / "meta_features.csv"
        if features_path.exists():
            result["technical_features"] = (
                pd.read_csv(features_path, header=None).iloc[:, 0].astype(str).tolist()
            )
